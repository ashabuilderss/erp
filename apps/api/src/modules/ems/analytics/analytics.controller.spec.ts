...
describe('AnalyticsController employee access', () => {
  it('forces employees to their own analytics record', async () => {
    ...
    await controller.getEmployeeAnalytics('self-employee', 'company-1', 'self-employee', 'EMPLOYEE');
    expect(service.getEmployeeAnalytics).toHaveBeenCalledWith('self-employee', 'company-1');
  });
  ...
  it('throws ForbiddenException when employee tries to access another employee analytics', async () => {
    ...
    await expect(
      controller.getEmployeeAnalytics('other-employee', 'company-1', 'self-employee', UserRole.EMPLOYEE)
  ...
  it('allows admin to access any employee analytics', async () => {
    ...
    await controller.getEmployeeAnalytics('other-employee', 'company-1', 'admin-employee', UserRole.ADMIN);
    expect(service.getEmployeeAnalytics).toHaveBeenCalledWith('other-employee', 'company-1');
  });
});
