...
describe('ReportsService', () => {
  const mockPrisma = () => ({
    ...
    booking: { count: jest.fn(), aggregate: jest.fn(), findMany: jest.fn(), groupBy: jest.fn() },
    ...
    reportExport: { findMany: jest.fn(), count: jest.fn(), create: jest.fn(), update: jest.fn() },
  ...
  describe('getCatalog', () => {
    it('returns catalog with 10 items', async () => {
      const service = new ReportsService(mockPrisma() as never, mockRedis() as never);
  ...
  describe('getKPIDashboard', () => {
    it('returns cached data when redis.get returns a value', async () => {
      ...
      const cached = { period: { dateFrom: '2024-01-01', dateTo: '2024-01-31' } };
      redis.get.mockResolvedValue(cached);
      const service = new ReportsService(prisma as never, redis as never);
    ...
    it('applies ownership filtering for EMPLOYEE', async () => {
      ...
      expect(prisma.lead.count).toHaveBeenCalledWith(expect.objectContaining({
        where: expect.objectContaining({ companyId: 'c1', assignedToEmployeeId: 'emp-1' }),
      }));
    });
  ...
  describe('createExport', () => {
    it('throws BadRequestException for unknown reportKey', async () => {
      ...
      await expect(service.createExport({ reportKey: 'unknown', format: 'CSV' } as never, 'c1', 'user-1')).rejects.toBeInstanceOf(BadRequestException);
    });

    it('creates export record, generates CSV data, returns base64', async () => {
      const prisma = mockPrisma();
      const redis = mockRedis();
      prisma.reportExport.create.mockResolvedValue({ id: 'exp-1', createdAt: new Date() });
      prisma.reportExport.update.mockResolvedValue({});
      prisma.employee.findMany.mockResolvedValue([]);
      const service = new ReportsService(prisma as never, redis as never);

      const result = await service.createExport({ reportKey: 'employees', format: 'CSV' } as never, 'c1', 'user-1');

      expect(result.status).toBe('COMPLETED');
      expect(result).toHaveProperty('csvData');
      expect(prisma.reportExport.update).toHaveBeenCalledWith(expect.objectContaining({
        where: { id: 'exp-1' },
    ...
    it('updates status to FAILED on generation failure', async () => {
      ...
      await expect(service.createExport({ reportKey: 'employees', format: 'CSV' } as never, 'c1', 'user-1')).rejects.toBeInstanceOf(BadRequestException);
  ...
  describe('getExports', () => {
    it('returns paginated data and meta', async () => {
      ...
      const service = new ReportsService(prisma as never, mockRedis() as never);
