...
let AnalyticsService = class AnalyticsService {
    ...
    async getDashboardStats(companyId) {
        const [totalProperties, propertiesByStatus, totalLeads, leadsByStatus, totalCustomers, totalSiteVisits, siteVisitsByStatus, totalBookings, bookingsByStatus, totalEmployees, activeEmployees, totalAssignments, assignmentsByType, avgPerformanceScore, topPerformers, attendanceRate, pendingLeaves, attendanceTrend, departmentDistribution,] = await Promise.all([
    ...
    async getEmployeeAnalytics(employeeId, companyId) {
        ...
        const [assignments, performance, attendance, leaves, propertiesAssigned, leadsAssigned, siteVisitsCompleted, bookingsClosed,] = await Promise.all([
        ...
        const presentDays = attendance.filter((a) => a.status === client_1.DayAggregateStatus.COMPLETED ||
            a.status === client_1.DayAggregateStatus.VERIFIED).length;
    ...
    async getTeamAnalytics(companyId, departmentId) {
        const where = { status: 'ACTIVE', companyId };
        if (departmentId)
        ...
        const avgScore = performance.length > 0
            ? Math.round(performance.reduce((sum, p) => sum + p.score, 0) /
                performance.length)
            : 0;
    ...
    async getConversionFunnel(companyId) {
        ...
        return {
            ...
            bookings,
            convertedLeads,
            leadToVisitRate: totalLeads > 0 ? Math.round((siteVisits / totalLeads) * 100) : 0,
            visitToBookingRate: siteVisits > 0 ? Math.round((bookings / siteVisits) * 100) : 0,
            leadToBookingRate: totalLeads > 0 ? Math.round((bookings / totalLeads) * 100) : 0,
        };
    }
    async getAttendanceRate(companyId) {
        const [total, present] = await Promise.all([
            this.prisma.attendanceDayAggregate.count({ where: { companyId } }),
            this.prisma.attendanceDayAggregate.count({
                where: {
                    status: { in: [client_1.DayAggregateStatus.COMPLETED, client_1.DayAggregateStatus.VERIFIED] },
                    companyId,
                },
            }),
        ]);
    ...
    getEmployeeAvgScore(performance, employeeId) {
        const empPerf = performance.filter((p) => p.employeeId === employeeId);
        return empPerf.length > 0
            ? Math.round(empPerf.reduce((sum, p) => sum + p.score, 0) / empPerf.length)
            : 0;
    }
    ...
    async getAttendanceTrend(companyId) {
        ...
        for (const r of records) {
            ...
            if (!entry)
                continue;
            if (r.status === client_1.DayAggregateStatus.COMPLETED ||
                r.status === client_1.DayAggregateStatus.VERIFIED)
                entry.present++;
            else if (r.status === client_1.DayAggregateStatus.UNDER_REVIEW)
                entry.absent++;
        }
