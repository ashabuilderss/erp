...
import { useBrokers, useCreateBroker, useUpdateBroker, useDeleteBroker } from "@/hooks/api";
...
export default function BrokersPage() {
  ...
  const resetForm = () => setForm({ name: "", companyName: "", phone: "", email: "", commissionRate: undefined, isActive: true });
  ...
  const columns: ColumnDef<Broker>[] = [
    ...
    { accessorKey: "commissionRate", header: "Commission %", cell: ({ row }) => <span>{row.original.commissionRate != null ? `${row.original.commissionRate}%` : "-"}</span> },
    { accessorKey: "isActive", header: "Status", cell: ({ row }) => <Badge variant="outline" className={row.original.isActive ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}>{row.original.isActive ? "Active" : "Inactive"}</Badge> },
    { id: "actions", header: "", cell: ({ row }) => (
      <div className="flex items-center gap-1">
        <Button variant="ghost" size="icon-sm" onClick={() => { setEditItem(row.original); setForm({ name: row.original.name, companyName: row.original.companyName ?? undefined, phone: row.original.phone ?? undefined, email: row.original.email ?? undefined, commissionRate: row.original.commissionRate ?? undefined, isActive: row.original.isActive }); }}><Pencil className="h-4 w-4" /></Button>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl font-semibold">Brokers</h2><p className="text-sm text-muted-foreground">Manage your brokers and their commission rates</p></div>
        <Dialog open={createOpen} onOpenChange={(o) => { setCreateOpen(o); if (!o) resetForm(); }}>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2"><Label>Name *</Label><Input value={form.name || ""} onChange={(e) => { setForm({ ...form, name: e.target.value } as Partial<Broker>); clearFieldError("name", setErrors); }} className={errors.name ? "border-red-500" : ""} /><FieldError error={errors.name} /></div>
              <div className="col-span-2"><Label>Company</Label><Input value={form.companyName || ""} onChange={(e) => setForm({ ...form, companyName: e.target.value } as Partial<Broker>)} /></div>
              <div><Label>Phone</Label><Input value={form.phone || ""} onChange={(e) => setForm({ ...form, phone: e.target.value } as Partial<Broker>)} /></div>
              <div><Label>Email</Label><Input value={form.email || ""} onChange={(e) => setForm({ ...form, email: e.target.value } as Partial<Broker>)} /></div>
              <div><Label>Commission Rate (%)</Label><Input type="number" value={form.commissionRate ?? ""} onChange={(e) => setForm({ ...form, commissionRate: e.target.value ? Number(e.target.value) : undefined } as Partial<Broker>)} /></div>
            </div>
            <DialogFooter><Button onClick={() => { const rules: ValidationRules<Broker> = { name: { required: "Name is required" } }; const errs = validateForm(form, rules); setErrors(errs); if (Object.keys(errs).length > 0) return; createMutation.mutate({ name: form.name!, companyName: form.companyName || undefined, phone: form.phone || undefined, email: form.email || undefined, commissionRate: form.commissionRate || undefined } as CreateBrokerDto, { onSuccess: () => { setErrors({}); showToast("Broker created"); setCreateOpen(false); resetForm(); }, onError: (err) => showToast(getApiErrorMessage(err, "Failed to create broker"), "error") }); }} disabled={createMutation.isPending}>Save</Button></DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
