...
import { useProperties, useCreateProperty, useUpdateProperty, useDeleteProperty, useEmployees } from "@/hooks/api";
  ...
  const columns: ColumnDef<Property>[] = [
    ...
    { id: "actions", header: "", cell: ({ row }) => (
      <div className="flex items-center gap-1">
        <Button variant="ghost" size="icon-sm" onClick={() => { setEditItem(row.original); setForm({ ...row.original, propertyCode: row.original.propertyCode ?? undefined, description: row.original.description ?? undefined, area: row.original.area ?? undefined, bedrooms: row.original.bedrooms ?? undefined, bathrooms: row.original.bathrooms ?? undefined, locality: row.original.locality ?? undefined, images: row.original.images ?? undefined, amenities: row.original.amenities ?? undefined, assignedToEmployeeId: row.original.assignedToEmployeeId ?? undefined } as PropertyForm); }}><Pencil className="h-4 w-4" /></Button>
        <Button variant="ghost" size="icon-sm" onClick={() => { if (confirm("Delete this property?")) deleteMutation.mutate(row.original.id, { onSuccess: () => showToast("Property deleted"), onError: (err) => showToast(getApiErrorMessage(err, "Failed to delete property"), "error") }); }}><Trash2 className="h-4 w-4 text-destructive" /></Button>
      </div>
    )},
  ];
    ...
    <div className="space-y-4">
        ...
        <Dialog open={createOpen} onOpenChange={(o) => { setCreateOpen(o); if (!o) resetForm(); }}>
          ...
          <DialogContent className="sm:max-w-lg">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2"><label className="text-sm font-medium">Title</label><Input value={form.title || ""} onChange={(e) => setForm({ ...form, title: e.target.value } as PropertyForm)} /></div>
              ...
              <div><label className="text-sm font-medium">Price</label><Input type="number" value={form.price || 0} onChange={(e) => setForm({ ...form, price: Number(e.target.value) } as PropertyForm)} /></div>
              ...
              <div><label className="text-sm font-medium">Location</label><Input value={form.location || ""} onChange={(e) => setForm({ ...form, location: e.target.value } as PropertyForm)} /></div>
              <div><label className="text-sm font-medium">City</label><Input value={form.city || ""} onChange={(e) => setForm({ ...form, city: e.target.value } as PropertyForm)} /></div>
              <div><label className="text-sm font-medium">State</label><Input value={form.state || ""} onChange={(e) => setForm({ ...form, state: e.target.value } as PropertyForm)} /></div>
            ...
            <DialogFooter showCloseButton><Button onClick={() => { if (!form.title || !form.price || !form.location || !form.city || !form.state) { showToast("Please fill Title, Price, Location, City, and State", "error"); return; } createMutation.mutate(form as CreatePropertyDto, { onSuccess: () => { showToast("Property created"); setCreateOpen(false); resetForm(); }, onError: (err) => showToast(getApiErrorMessage(err, "Failed to create property"), "error") }); }} disabled={createMutation.isPending || uploading}>Save</Button></DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
      ...
      <Dialog open={!!editItem} onOpenChange={(o) => { if (!o) setEditItem(null); }}>
        <DialogContent className="sm:max-w-lg">
          ...
          <DialogFooter showCloseButton><Button onClick={() => { if (editItem) { updateMutation.mutate({ id: editItem.id, dto: form as UpdatePropertyDto }, { onSuccess: () => { showToast("Property updated"); setEditItem(null); }, onError: (err) => showToast(getApiErrorMessage(err, "Failed to update property"), "error") }); } }} disabled={updateMutation.isPending || uploading}>Save</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
